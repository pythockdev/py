...
╰─⍟ \33[92m"""+str(hitr)+"""SuDevHits """ +str(hitc)+"""  \33[0m\33[91mRᴜɴ➢"""+str(oran)+"""%   \33[0m
""")
...
"[■■■■■■■■■■■■■■■]"]
	...
	time.sleep(0.2)
	sys.stdout.write("\r"'\33[91mSuDev\33[0m'+ animation[ i % len(animation)]+'    \33[91mPʀᴏxCʜᴇᴄᴋ️   \33[0m')
	sys.stdout.flush()
	#print('\n')			

		
kanalkata="2"
stalker_portal="sudev"
...
├◉𝐌𝐚𝐜 ➤ """+str(mac)+"""
├◉𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝙂𝙧𝙤𝙪𝙥 ➤https://t.me/+fRvhMpHX42AyZTY1
├◉𝐄𝐱𝐩 ➤ """+str(trh)+"""
...
if not os.path.exists(hits):
    os.mkdir(hits)
Dosyab=hits+"SuDevHits_"+dosyaadi+"@sudev.txt"
hitsay=0
say=1
def yax(hits):
    dosya=open(Dosyab,'a+') 
...
import threading
for SuDevHits in range(int(botgir)):
	SuDevHits = threading.Thread(target=XD)
	SuDevHits.start()
